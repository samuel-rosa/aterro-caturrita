...
diag(cor1) <- NA_real_
cor.colors <- colorRampPalette(c("blue", "white", "red"))
png("../res/fig/corrplot.png", width = 480 * 2, height = 480 * 2, res = 72 * 2)
par(mar = c(4, 4, 4, 4) + 0.1)
image(t(cor1)[ncol(cor1):1, ], axes = FALSE, col = cor.colors(200), breaks = seq(-1, 1, by = 0.01))
box()
text(
...
axis(side = 1, at = seq(0, 1, length.out = ncol(cor1)), labels = rev(colnames(cor1)), las = 2)
axis(side = 2, at = seq(0, 1, length.out = ncol(cor1)), labels = colnames(cor1), las = 1)
axis(side = 3, at = seq(0, 1, length.out = ncol(cor1)), labels = rev(colnames(cor1)), las = 2)
axis(side = 4, at = seq(0, 1, length.out = ncol(cor1)), labels = colnames(cor1), las = 1)
dev.off()
```
